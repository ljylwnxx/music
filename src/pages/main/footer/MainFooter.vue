        ...
        <div class="play-view">
          <div class="play-music-info">
            ...
            <div class="lyric-view">
              ...
              <LyricWrap
                :musicId="$store.state.musicData.id"
...
export default {
  name: 'MainFooter',
  components: {LyricWrap, Comment, MusicList},
  data () {
    return {
      volume: 30,
      isMute: true,
  ...
  methods: {
    ...
    getMusicUrl (musicData) {
      console.log(musicData, 'musicData')
      let music = {}
      let currentIndex = ''
      musicData.musicList.forEach((item, index) => {
      ...
      httpGet(`/api/song/url?id=${music.id}`).then((res) => {
        this.$store.state.currentMusicInfo = {
          ...music,
          musicUrl: res.data.data[0].url,
          lastId: (musicData.musicList[currentIndex - 1] || {}).id,
          nextId: (musicData.musicList[currentIndex + 1] || {}).id
        }
        setTimeout(() => {
          this.isPlay = true
    ...
    likeMusic () {
      ...
      httpGet(`/api/like?id=${this.musicDetail.id}&like=${!this.isLiked}`).then((res) => {
